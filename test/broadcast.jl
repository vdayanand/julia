...
Broadcast.rule(::Type{T}) where {T<:AD1} = AD1
Base.similar(f, r::Broadcast.Result{AD1}, As...) =
    AD1(Array{eltype(r)}(indices(r)))
struct AD2{T,N} <: ArrayData{T,N}
    data::Array{T,N}
end
Broadcast.rule(::Type{T}) where {T<:AD2} = AD2
Base.similar(f, r::Broadcast.Result{AD2}, As...) =
    AD2(Array{eltype(r)}(indices(r)))

# Two specialized broadcast rules with explicit precedence
struct AD1P{T,N} <: ArrayData{T,N}
    data::Array{T,N}
end
Broadcast.rule(::Type{T}) where {T<:AD1P} = AD1P
Base.similar(f, r::Broadcast.Result{AD1P}, As...) =
    AD1P(Array{eltype(r)}(indices(r)))
struct AD2P{T,N} <: ArrayData{T,N}
    data::Array{T,N}
end
Broadcast.rule(::Type{T}) where {T<:AD2P} = AD2P
Base.similar(f, r::Broadcast.Result{AD2P}, As...) =
    AD2P(Array{eltype(r)}(indices(r)))

Broadcast.rule(::Type{AD1P}, ::Type{AD2P}) = AD1P

# Two specialized broadcast rules where users unnecessarily
# define `rule` for both argument orders (but do so consistently)
struct AD1B{T,N} <: ArrayData{T,N}
    data::Array{T,N}
end
Broadcast.rule(::Type{T}) where {T<:AD1B} = AD1B
Base.similar(f, r::Broadcast.Result{AD1B}, As...) =
    AD1B(Array{eltype(r)}(indices(r)))
struct AD2B{T,N} <: ArrayData{T,N}
    data::Array{T,N}
end
Broadcast.rule(::Type{T}) where {T<:AD2B} = AD2B
Base.similar(f, r::Broadcast.Result{AD2B}, As...) =
    AD2B(Array{eltype(r)}(indices(r)))

Broadcast.rule(::Type{AD1B}, ::Type{AD2B}) = AD1B
Broadcast.rule(::Type{AD2B}, ::Type{AD1B}) = AD1B

# Two specialized broadcast rules with conflicting precedence
struct AD1C{T,N} <: ArrayData{T,N}
    data::Array{T,N}
end
Broadcast.rule(::Type{T}) where {T<:AD1C} = AD1C
Base.similar(f, r::Broadcast.Result{AD1C}, As...) =
    AD1C(Array{eltype(r)}(indices(r)))
struct AD2C{T,N} <: ArrayData{T,N}
    data::Array{T,N}
end
Broadcast.rule(::Type{T}) where {T<:AD2C} = AD2C
Base.similar(f, r::Broadcast.Result{AD2C}, As...) =
    AD2C(Array{eltype(r)}(indices(r)))

Broadcast.rule(::Type{AD1C}, ::Type{AD2C}) = AD1C
Broadcast.rule(::Type{AD2C}, ::Type{AD1C}) = AD2C
...
@testset "treat type arguments as scalars, DataArrays issue 229" begin
    @test Broadcast.combine_types(AbstractArray) == Broadcast.Scalar
