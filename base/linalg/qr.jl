...
full(F::Union{QR,QRCompactWY}) = AbstractArray(F)
...
full(F::QRPivoted) = AbstractArray(F)
...
Matrix(A::AbstractQ{T}) where {T} = A_mul_B!(A, eye(T, size(A.factors, 1), min(size(A.factors)...)))
